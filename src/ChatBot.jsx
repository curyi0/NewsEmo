...
const Chatbot = () => {
  ...
  const SearchNews = async () => {
    try {
      const response = await axios.get(`${Base_URL}/api/chatbot/search/news`, {
      ...
      if (response.status === 200) {
        ...
        console.log('뉴스 데이터:', newsList); // 데이터 구조 확인용

        // if (Array.isArray(newsList) && newsList.length > 0) {
        //   // 최신 뉴스 2개만 가져오기
        //   const latestNews = newsList.slice(0, 2);
        //   let newsMessage = "📰 최신 뉴스 2개입니다:\n\n";

        //   latestNews.forEach((news, index) => {
        //     newsMessage += `${index + 1}. ${news.title || '제목 없음'}\n`;
        //     if (news.summary) {
        //       newsMessage += `   요약: ${news.summary}\n`;
        //     }
        //     if (news.published_at) {
        //       newsMessage += `   날짜: ${news.published_at}\n`;
        //     }
        //     newsMessage += "\n";
        //   });
